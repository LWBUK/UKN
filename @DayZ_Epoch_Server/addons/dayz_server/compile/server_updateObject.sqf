private ["_object","_type","_objectID","_uid","_lastUpdate","_needUpdate","_object_position","_isnormal","_object_inventory","_object_damage","_isNotOk","_parachuteWest","_firstTime","_object_killed","_object_repair","_isbuildable"];
...
_foldchair = ((typeOf _object == "FoldChair_with_Cargo") || (typeOf _object == "FoldChair_with_Cargo"));
...
if (!_parachuteWest && !_foldchair && !(locked _object)) then {
...
_object_position = {
    private["_position","_worldspace","_fuel","_key","_colour","_colour2"];
        ...
        if (_object isKindOf "AllVehicles") then {
            ...
            _worldspace = toArray _worldspace;
            _worldspace resize (count _worldspace - 1);
            _worldspace = toString _worldspace + format [",%1,%2]", _colour, _colour2];
            _fuel = fuel _object;
        } else {
            _worldspace = [getDir _object, _position] call AN_fnc_formatWorldspace;
            _fuel = 0;
