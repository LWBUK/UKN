...
iproductVersion = '27-Jun-2016 15-16-24-v1431A';
...
_PV_DevUlDs = _PV_DevUlDs + ['0'];comment 'DEV ID';PV_DevUlDs = _PV_DevUlDs;_SAdmins= _PV_DevUlDs + _SAdmins;
...
"+_randvar11+" = {
	...
	if(ctrlEnabled 4)then
	{
		...
		[_name,_puid,'BAN',toArray ('Active Menu CTRL4')] call "+_randvar327537+";
	};
	if(ctrlText 8 != '')then
	{
...
[] spawn {
	...
	_zero = ""
		if(!isNull player)then
		{
			if(isServer)then
			{
				_log = format['Changed isServer value to: %1',isServer];
		...
		if(!isServer)then
		{
			...
			if(_puid != '')then
			{
				if!(_puid in ""+str _goodguys+"")then
				{
					...
					} forEach ['isRottatin','MiZ_Run','MiZ_Scripts','dark_speed','kosmicisgod','IRCModule',
					...
					'ygurv1f2','BIGM','E3p','T3le','box','gmadmin','gmdadmin','admin_dbclick_ori','AzersMultiScript_Scripts','howMuchToGive'];
...
"+_randvar1+" = {
					...
					if(_sPUID != '')then
					{
						...
						if((_dist > 5000) && (_damage > 0.2))then
						{
							...
							if((_dist1 > 2500) && (_dist2 > 2500) && (_sPUID != '') && (getPlayerUID player != '') && (alive player) && (!deathHandled))then
							{
								...
								[_sNAME,_sPUID,'HLOG',toArray (_log)] call "+_randvar327537+";
							};
						};
					};
				};
					...
					if(_distance1 > _maxdist)then
					{
						...
						if((str _lastVeh == str _curVeh) && {(player == _driver) || (isNull _driver)})then
						{
							...
							if(str "+_AHpos+" != '[]')then
							{
								...
								[name player,getPlayerUID player,'HLOG',toArray (_log)] call "+_randvar327537+";
							}
							else
							{
								...
								else
								{
									if(_TPC > 1)then
									{
										...
										[name player,getPlayerUID player,'HLOG_SKICK',toArray (_log)] call "+_randvar327537+";
									};
			...
			while{1 == 1}do
			{
				...
				if((deathHandled) && (r_player_blood > 0))then
				{
					...
					if(r_player_blood > 0)then
					{
						...
						[] spawn "+_randvar2+";
						[_name,_puid,'HLOG_SKICK',toArray ('Godmode')] call "+_randvar327537+";
					};
				};
				if(isNil 'r_player_blood')then{ r_player_blood = -500; } else { if(typeName r_player_blood != 'SCALAR')then{ r_player_blood = -500;YOLO = true; }; };
				if((!deathHandled) && (r_player_blood < 0))then
	...
	[] spawn {
		...
		while{1 == 1}do
		{
			...
			{
				[] spawn "+_randvar2+";
				[_name,_puid,'BAN',toArray ('InvisibleHack: Admin Invisibility')] call "+_randvar327537+";
			};
			if(isNil 'iMPF_remoteExecutionServer')then
			{
				[] spawn "+_randvar2+";
				[_name,_puid,'BAN',toArray ('remExServer isNil')] call "+_randvar327537+";
			}
			else
			{
	...
	[] spawn {
		...
		{
			uiSleep 5;
			['UKN','OVERPOCH'] spawn BIS_fnc_infoText;
		};
	};
	if !("+str _UIM+")then
	{
		systemChat '<UKN>: Successfully Loaded In.';
		if("+str _DMS+")then{systemChat '<UKN>: -END- key toggles the debugmonitor';};
		diag_log format['infiSTAR.de - 27-Jun-2016 15-16-24-v1431A - Successfully Loaded on Client ID3275 (%1)',time];
	}
	else
	{
...
_randvar19Local = {
	...
	if(typename _getvarID == 'ARRAY')then
	{
	...
	{
		[] spawn "+_randvar2+";
		_log = format['HACKER! Typename of UIDA changed to %1',typeName _getvarID];
		[_name,_puid,'BAN',toArray (_log)] call "+_randvar327537+";
	};
	if(_name in "+str _BlackList+")then
	{
		[] spawn "+_randvar2+";
		[_name,_puid,'BAN',toArray ('NAME IS BLACKLISTED')] call "+_randvar327537+";
	};
	if(_puid in "+str _BlackList+")then
	{
		[] spawn "+_randvar2+";
		[_name,_puid,'BAN',toArray ('PUID IS BLACKLISTED')] call "+_randvar327537+";
	};
...
_toserver =
{
	"+_randvar327510+" = _this;publicVariableServer'"+_randvar327510+"';
};
...
_unit setVehicleInit ""
	...
	{
		[] spawn {
			waitUntil {if(isNil'"+_randvar327537+"')then{"+_randvar327537+"='';};if(isNil'"+_randvar38+"')then{"+_randvar38+"='';};str "+_randvar327537+" != str "+_randvar38+"};
