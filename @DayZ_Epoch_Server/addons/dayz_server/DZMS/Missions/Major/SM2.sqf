...
[_coords,6,1,format ["DZMSUnitsMajor%1", _code]] call DZMSAISpawn;
sleep 5;
[_coords,6,1,format ["DZMSUnitsMajor%1", _code]] call DZMSAISpawn;
sleep 5;
[_coords,4,1,format ["DZMSUnitsMajor%1", _code]] call DZMSAISpawn;
sleep 5;
...
while {_loop} do {
	if (!Alive _plane OR !Alive _pilot) then {
		...
		//Update the location
		[_coords,"AN2 Wreck",_code] ExecVM DZMSAddMajMarker;
...
//[[(getpos _boxFin select 0), (getpos _boxFin select 1), 0],"AN2 Cargo",_code] ExecVM DZMSAddMajMarker;
[_coords,"AN2 Cargo",_code] ExecVM DZMSAddMajMarker;
clearWeaponCargoGlobal _boxFin;
clearMagazineCargoGlobal _boxFin;
clearBackpackCargoGlobal _boxFin;
[_boxFin,"supply"] ExecVM DZMSBoxSetup; //DONN changed from weapons to supply
[_boxFin] call DZMSProtectObj;

//Wait until the player is within 30 meters and also meets the kill req
[position _boxFin,format ["DZMSUnitsMajor%1", _code]] call DZMSWaitMissionComp;
