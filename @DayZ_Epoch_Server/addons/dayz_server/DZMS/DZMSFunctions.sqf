...
DONN_DZMS_findPos_running = false;
DZMSFindPos = {
    private["_mapHardCenter","_mapRadii","_isTavi","_centerPos","_pos","_disCorner","_hardX","_hardY","_findRun","_posX","_posY","_feel1","_feel2","_feel3","_feel4","_noWater","_tavTest","_tavHeight","_disMaj","_disMin","_okDis","_isBlack","_code","_majMin"];
	
	_majMin = _this select 0;
    ...
    if (_mapHardCenter AND (!(DZMSStaticPlc))) then {
        ...
        while {_findRun} do
        {
			...
			//DONN not needed because missions add a Black List areas to prevent near missions
		   ...
		   _okDis = true;
            ...
            if ((_posX != _hardX) AND (_posY != _hardY) AND _noWater AND _okDis AND !_isBlack) then {
				if (!(_isTavi) || (_isTavi AND (_tavHeight <= 185))) then {
					_findRun = false;
					if (_majMin == "Maj") then {
						DZMS_MajBlack set [_code-1, [_pos,400]];
	...
	if (DZMSStaticPlc) then {
		private ["_element","_notIn"];
		_notIn = false;
		while {!_notIn} do {
			_element = floor (random ((count DZMSStatLocs)-0.01));
			_pos = DZMSStatLocs select _element;
			_notIn = true;
			{
				if (_pos select 0 == _x select 0 && _pos select 1 == _x select 1) exitWith {
					_notIn = false;
				};
			} forEach DONN_DZMS_staticUSed;
		};
		diag_log format ["[DONN DZMS] _pos %1 / % StaticUsed %2 / in %3", _pos, DONN_DZMS_staticUSed];
		DONN_DZMS_staticUSed set [_element, _pos];
	};
   
    _fin = [(_pos select 0), (_pos select 1), 0];
    call compile format["%1%2_coord = _fin;", _majMin, _code];
	_fin
};
