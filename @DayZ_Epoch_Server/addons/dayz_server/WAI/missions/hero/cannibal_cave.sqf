...
_baserunover23 = createVehicle ["UralWreck", [(_position select 0) - 41.3, (_position select 1) - 26,-0.02],[], 0, "CAN_COLLIDE"];
	...
	[[(_position select 0) + 11, (_position select 1) + 41, 0],2,"hard","random",4,"random","TK_Special_Forces_EP1","random","bandit",_mission] call spawn_group;
	[[(_position select 0) - 12, (_position select 1) - 43, 0],2,"random","random",4,"random","TK_Special_Forces_EP1","random","bandit",_mission] call spawn_group;
	...
	if(_complete) then {
		[_crate,[8,ai_wep_all],[8,crate_tools_sniper],[3,crate_items_high_value],[4,crate_backpacks_large]] call dynamic_crate;
	};	
