...
_baserunover13 = createVehicle ["Land_Fire_barrel_burning",[(_position select 0) - 9, (_position select 1) - 1,-0.02],[], 0, "CAN_COLLIDE"];
	...
	//Troops
	[[(_position select 0) - 7, (_position select 1) - 10, 0],2,"Hard",["Random","AT"],4,"Random","TK_Special_Forces_TL_EP1","Random","Bandit",_mission] call spawn_group;
	[[(_position select 0) + 16, (_position select 1) - 5, 0],2,"Hard","Random",4,"Random","TK_INS_Soldier_AT_EP1","Random","Bandit",_mission] call spawn_group;
	[[(_position select 0) + 4, (_position select 1) + 18, 0],2,"Hard","Random",4,"Random","TK_INS_Soldier_MG_EP1","Random","Bandit",_mission] call spawn_group;
	...
	if(_complete) then {
		[_crate,[8,ai_wep_all],5,1,2] call dynamic_crate;
			};
