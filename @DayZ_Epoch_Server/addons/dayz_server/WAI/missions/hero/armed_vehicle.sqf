if(isServer) then {
	...
	[[_position select 0,_position select 1,0],_rndnum,"extreme",["Random","AT"],3,"Random","Bandit","Random","Bandit",_mission] call spawn_group;
	[[_position select 0,_position select 1,0],_rndnum,"Medium","Random",3,"Random","Bandit","Random","Bandit",_mission] call spawn_group;
	[[_position select 0,_position select 1,0],_rndnum,"extreme","Random",3,"Random","Bandit","Random","Bandit",_mission] call spawn_group;
	...
	if(_complete) then {
		[_crate,[8,ai_wep_all],4,[25,crate_items_chainbullets],2] call dynamic_crate;
	};
