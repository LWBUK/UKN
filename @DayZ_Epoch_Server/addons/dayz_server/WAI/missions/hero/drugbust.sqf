if(isServer) then {
	...
	diag_log 		format["WAI: [Mission:[Hero] Drug Den]: Starting... %1",_position];
...
_baserunover23 = createVehicle ["MAP_t_salix2s", [(_position select 0) - 24, (_position select 1) - 0.01,-0.01],[], 0, "CAN_COLLIDE"];
	...
	//Troops
	[[(_position select 0) - 12, (_position select 1) - 15,0],2,"Medium",["Random","AT"],4,"Random","Bandit","Random","Bandit",_mission] call spawn_group;
	[[(_position select 0) + 12, (_position select 1) + 15,0],2,"Medium","Random",4,"Random","Bandit","Random","Bandit",_mission] call spawn_group;
	...
	_complete = [
		...
		"Bandits have set up a Drug Den! Check your map for the location!",	// mission announcement
		"Survivors have secured the Drug Den!",																	// mission success
		"Survivors did not secure Drug Den in time"															// mission fail
	] call mission_winorfail;

	diag_log format["WAI: [Mission:[Hero] Crack House]: Ended at %1",_position];

	if(_complete) then {
		[_crate,[8,ai_wep_all],5,2,2] call dynamic_crate;
	};
