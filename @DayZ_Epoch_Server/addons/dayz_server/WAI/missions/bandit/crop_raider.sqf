...
[[(_position select 0) + 13, (_position select 1) + 15, 0],2,"Hard","Random",4,"Random","Rocker1_DZ","Random","Hero",_mission] call spawn_group;
[[(_position select 0) - 23, (_position select 1) - 25, 0],2,"Hard","Random",4,"Random","(Rocker2_DZ","Random","Hero",_mission] call spawn_group;
...
//Heli Paradrop
[[(_position select 0), (_position select 1), 0],200,"UH1H_DZ",5,"Hard","Random",4,"Random","Hero","Random","Hero",true,_mission] spawn heli_para;
...
"Survivors did not stop the bandits in time." // mission fail
  ...
  if(_complete) then {
  [_crate,[8,ai_wep_all],5,2,3] call dynamic_crate;
};
