...
_baserunover42 = createVehicle ["Land_Fire_barrel_burning",[(_position select 0) - 0.01, (_position select 1) - 0.01,-0.02],[], 0, "CAN_COLLIDE"];
	...
	//Troops
	[[(_position select 0) - 12, (_position select 1) + 2, 0],5,"extreme",["Random","AT"],4,"Random","Hero","Random","Hero",_mission] call spawn_group;
	[[(_position select 0) + 2, (_position select 1) + 2, 0],5,"extreme","Random",4,"Random","Hero","Random","Hero",_mission] call spawn_group;
	[[(_position select 0) + 14, (_position select 1) - 35, 0],3,"Hard","Random",4,"Random","Hero","Random","Hero",_mission] call spawn_group;
	...
	if(_complete) then {
		[_crate,[8,ai_wep_all],5,[2,crate_items_high_value],2] call dynamic_crate;
			};
