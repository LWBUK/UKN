...
_baserunover8 = createVehicle ["MAP_t_quercus3s",[(_position select 0) + 22, (_position select 1) + 25,-0.2],[], 0, "CAN_COLLIDE"];
	...
	[[(_position select 0) + 12, (_position select 1) + 22.5, 0],1,"extreme",["random","at"],4,"random","TK_INS_Soldier_AT_EP1","random",["Hero",150],_mission] call spawn_group;
	[[(_position select 0) + 21, (_position select 1) + 11, 0],2,"hard","random",4,"random","TK_Special_Forces_EP1","random","Hero",_mission] call spawn_group;
	[[(_position select 0) - 1.12, (_position select 1) - 0.43, 0],2,"random","random",4,"random","TK_Special_Forces_EP1","random","Hero",_mission] call spawn_group;
	...
	if(_complete) then {
		[_crate,[8,ai_wep_all],[8,crate_tools_sniper],[10,crate_items_buildables],[4,crate_backpacks_large]] call dynamic_crate;
	};	
