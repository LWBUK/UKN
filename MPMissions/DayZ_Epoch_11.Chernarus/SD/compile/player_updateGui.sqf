...
if (DZE_kick_before_restart) then {
	...
	if ((_Serverhours == _ServerKickhours)&&(_Serverminutes == _ServerKickminutes)) exitWith {
		[] spawn {
		["<t size='1' shadow ='.6' color='#B22222'> Restart detected! Logging you out to prevent gear loss</t>",0,(safezoneY + safezoneH * 0.43),30,1,0,32] spawn bis_fnc_dynamicText;
			_msg = "Restart detected! Logging you out to prevent gear loss..";
			systemChat ("[UKN] "+str _msg);
