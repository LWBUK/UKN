...
if (_dikCode == 0x01) then {
	...
	call dayz_EjectPlayer;
	[] execVm "SD\functions\esc.sqf";
};

/* Anti-Duping by JustBullet */
if (_dikCode in actionKeys "Gear") then {
    _nill = execvm "SD\functions\gear.sqf";
};
...
if (_dikCode == 0x0B) then {
	[]execVM "SD\functions\autorun.sqf";
};
...
if (_dikCode == 0x16) then
    {
        if (soundVolume == 1) then {
            1 fadeSound 0.25;
            cutText ['Earplugs Inserted', 'PLAIN'];
        }
            else
        {
            1 fadeSound 1;
            cutText ['Earplugs Removed', 'PLAIN'];
        };
    };
...
if (_dikCode == 0x15) then {
	if (vehicle player == player) then {
		if ((animationState player) in ["amovpercmevasraswrfldf","amovpercmevasraswpstdf","amovpercmevasnonwnondf","amovpercmevasraswpstdfl","amovpercmevasraswpstdfr","amovpercmrunsraswpstdf"]) then {
			player switchMove "";
...
_null = execVM "SD\functions\rules.sqf";
};
