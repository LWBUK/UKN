...
"_location","_dir","_classname","_item","_hasrequireditem","_missing","_hastoolweapon","_cancel","_reason","_started","_finished","_animState","_isMedic","_dis","_sfx","_hasbuilditem","_tmpbuilt","_onLadder","_isWater","_require","_text","_offset","_IsNearPlot","_isOk","_location1","_location2","_counter","_limit","_proceed","_num_removed","_position","_object","_canBuildOnPlot","_friendlies","_nearestPole","_ownerID","_findNearestPoles","_findNearestPole","_distance","_classnametmp","_ghost","_isPole","_needText","_lockable","_zheightchanged","_rotate","_combination_1","_combination_2","_combination_3","_combination_4","_combination","_combination_1_Display","_combinationDisplay","_zheightdirection","_abort","_isNear","_need","_needNear","_vehicle","_inVehicle","_requireplot","_objHDiff","_isLandFireDZ","_isTankTrap","_playerID", "_playerUID","_ownerID","_buildcheck","_isowner","_isfriendly"];
...
_isGarage = (_classname == "Land_MBG_Garage_Single_A");
...
// check for near plot
_findNearestGarages = nearestObjects [(vehicle player), ["Land_MBG_Garage_Single_A"], _distance];
_findNearestGarage = [];

{
    if (alive _x) then {
        _findNearestGarage set [(count _findNearestGarage),_x];
    };
} count _findNearestGarages;

_IsNearGarage = count (_findNearestGarage);

// If item is plot pole && another one exists within 45m                     
if(_isGarage && _IsNearGarage > 0) exitWith {  DZE_ActionInProgress = false; cutText [format["You can only build one garage per plot."], "PLAIN DOWN"];};
...
if (_hasrequireditem) then {
	...
	if !(_classname in DZE_noRotate) then{
		s_player_setVectorsReset = player addaction ["Reset","SD\actions\player_vectorChange.sqf","reset"];
		s_player_setVectorsForward = player addaction ["Pitch Forward","SD\actions\player_vectorChange.sqf","forward"];
		s_player_setVectorsBack = player addaction ["Pitch Back","SD\actions\player_vectorChange.sqf","back"];
		s_player_setVectorsLeft = player addaction ["Bank Left","SD\actions\player_vectorChange.sqf","left"];
		s_player_setVectorsRight = player addaction ["Bank Right","SD\actions\player_vectorChange.sqf","right"];
		s_player_setVectors1 = player addaction ["Increment by 1 degree","SD\actions\player_vectorChange.sqf","1"];
		s_player_setVectors5 = player addaction ["Increment by 5 degrees","SD\actions\player_vectorChange.sqf","5"];
		s_player_setVectors45 = player addaction ["Increment by 45 degrees","SD\actions\player_vectorChange.sqf","45"];
		s_player_setVectors90 = player addaction ["Increment by 90 degrees","SD\actions\player_vectorChange.sqf","90"];
	};
            ...
            //###BEGIN MODIFIED CODE
                ...
                if(_lockable > 1) then {
                    ...
                    if (_index call getPermanent) then {
                        ...
                        if(_index call getDeployableSimulation) then {
                            PVDZE_veh_Publish = [_tmpbuilt,[_dir,_location,_vector],(_index call getDeployableClass),true,call fnc_perm_deployable_id];
