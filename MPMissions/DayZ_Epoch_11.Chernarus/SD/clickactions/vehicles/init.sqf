// included compiles
call compile preprocessFileLineNumbers "SD\ClickActions\vehicles\config.sqf";
call compile preprocessFileLineNumbers "SD\ClickActions\vehicles\wrapper.sqf";
call compile preprocessFileLineNumbers "SD\ClickActions\vehicles\functions.sqf";
...
player_deploy = compile preprocessFileLineNumbers "SD\ClickActions\vehicles\player_deploy.sqf";
...
[] spawn {
    ...
    // call dependency
    call compile preprocessFileLineNumbers "SD\ClickActions\init.sqf";
    ...
    {DZE_CLICK_ACTIONS = DZE_CLICK_ACTIONS + [[(_forEachIndex call getDeployableKitClass),format["Deploy %1",(_forEachIndex call getDeployableDisplay)],format["%1 execVM 'SD\ClickActions\vehicles\deploy.sqf';",_forEachIndex],(_forEachIndex call getDeployableCondition)]];} forEach DZE_DEPLOYABLES;
    DZE_DEPLOYING      = false;
    DZE_PACKING        = false;
            ...
            {   
                ...
                if(!(isNull _cursorTarget)
                    ...
                    if ((_forEachIndex call getActionId) < 0) then {
                        [_forEachIndex,player addaction["<t color='#33b5e5'>" + format["Pack %1",(_forEachIndex call getDeployableDisplay)] + "</t>","SD\ClickActions\vehicles\pack.sqf",[_forEachIndex,_cursorTarget],0,false,true,"", ""]] call setActionId;
                    };
                } else {
