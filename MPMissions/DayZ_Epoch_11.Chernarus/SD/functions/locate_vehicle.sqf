...
if (_showMapMarker && !("ItemMap" in _inv)) exitWith { systemChat "<UKN> You need a map to locate your vehicles!";};
_keyColor = [];
_keyID = [];
_removedID = [];
_count = 0;
...
if (_count == 0) exitWith { systemChat "<UKN> No key found!";};

if (_count == 1) then { systemChat format ["<UKN> Found: %1 vehicle key",_count];}
else {systemChat format ["<UKN> Found: %1 vehicle keys",_count];};
...
for "_i" from 0 to _count do {
	...
	if (_found != 0) then {
		...
		if (_showMapMarker) then {
			...
			_vehicleMarker setMarkerTextLocal format ["<UKN> Here is your: %1",_vehicleName];
			systemChat format ["%1 belongs to %2 - %3",_keyName,_vehicleName,_finalID];
		} else { systemChat format ["<UKN> %1 belongs to %2 - %3",_keyName,_vehicleName,_finalID];};
	} else { systemChat format ["<UKN> %1 - Vehicle ID: %2 - (This vehicle no longer exists in the database)",_keyName,_finalID];};	
};

if (_showMapMarker) then {
	if (_count == 0) then { systemChat "<UKN> Check the map to locate your vehicle!";}
	else {systemChat "<UKN> Check the map to locate your vehicles!";};
};